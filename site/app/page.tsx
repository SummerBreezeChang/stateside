...
function LandingScreen({ onStart, savedCount }: { onStart: () => void; savedCount: number }) {
  return <><AppHeader screen="landing" /><main>
    <section className="m-3 overflow-hidden rounded-[1.75rem] bg-gradient-to-br from-teal-950 via-teal-900 to-slate-900 text-white sm:m-5">
      <div className="mx-auto grid min-h-[76vh] max-w-6xl gap-10 px-5 py-16 sm:px-8 sm:py-24 lg:grid-cols-[1.15fr_.85fr] lg:items-center">
        <div><Badge tone="sample">Housing decisions for international students</Badge><h1 className="mt-6 max-w-3xl text-4xl font-semibold tracking-tight sm:text-6xl">Compare three rentals. Know what to verify before you pay.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-teal-50"><strong className="font-semibold text-white">Stateside is an AI housing decision companion.</strong> Paste the places you found and it turns scattered listing details into one evidence-labeled comparison—qualification, true cost, lease fit, rest, location, and questions to ask.</p><div className="mt-8 flex flex-wrap items-center gap-3"><Button variant="secondary" size="lg" onClick={onStart}>Try the Berkeley demo →</Button><a href="#how-it-works" className="rounded-md px-4 py-3 text-sm font-semibold text-teal-50 underline-offset-4 hover:underline">See how it works</a></div><p className="mt-4 text-sm text-teal-100">No account, payment, or personal documents required</p></div>
        <div className="stateside-flow overflow-hidden rounded-[1.5rem] border border-white/20 bg-[#f6f2e9] shadow-2xl shadow-black/20"><img src="/stateside-flow-v2.jpg" alt="Photographic concept of three rental listings becoming one organized comparison" loading="eager" className="aspect-[16/10] h-full w-full object-cover" /><div className="border-t border-stone-200 bg-white px-5 py-4 text-stone-900"><p className="text-sm font-semibold">Three scattered listings → one clear decision plan</p><p className="mt-1 text-xs text-stone-500">Visual concept of the Stateside workflow</p></div></div>
      </div>
    </section>
    <section id="how-it-works" className="mx-auto max-w-6xl scroll-mt-24 px-5 py-16 sm:px-8 sm:py-24"><p className="text-sm font-semibold text-teal-900">How to use Stateside</p><h2 className="mt-2 max-w-3xl text-3xl font-semibold tracking-tight sm:text-5xl">From a confusing shortlist to clear next steps.</h2><p className="mt-5 max-w-2xl text-lg leading-8 text-stone-600">Use places you already found on Craigslist, a university portal, or any rental site. Stateside helps with the decision after discovery and before payment.</p><div className="mt-10 grid gap-5 md:grid-cols-3">{[["1", "Add your shortlist", "Paste the listing, landlord message, or lease excerpt for up to three places."], ["2", "Add your situation", "Tell Stateside your school dates, qualification situation, transportation, and priorities."], ["3", "Compare and verify", "See the same questions for every place, then open a plan with documents, landlord questions, and an inquiry email."]].map(([number, title, copy]) => <Card className="overflow-hidden" key={title}><StepVisual step={number} /><div className="p-6"><div className="flex items-center gap-3"><span className="grid h-8 w-8 place-items-center rounded-full bg-teal-900 text-sm font-semibold text-white">{number}</span><h3 className="text-xl font-semibold">{title}</h3></div><p className="mt-4 text-sm leading-6 text-stone-600">{copy}</p></div></Card>)}</div></section>
    <section id="what-you-get" className="border-y border-stone-200 bg-white"><div className="mx-auto grid max-w-6xl gap-10 px-5 py-16 sm:px-8 sm:py-24 lg:grid-cols-[.9fr_1.1fr] lg:items-center"><div><p className="text-sm font-semibold text-teal-900">What Stateside does</p><h2 className="mt-2 text-3xl font-semibold tracking-tight sm:text-5xl">It answers the questions a listing page leaves open.</h2><p className="mt-5 text-lg leading-8 text-stone-600">Rental sites help you find options. Stateside helps you understand the commitment behind the options.</p><div className="mt-8 grid gap-3">{[["qualification", "Can I qualify?", "Check the stated route before paying an application fee."], ["cash", "What will I really pay?", "Normalize rent, recurring costs, fees, and move-in cash."], ["verification", "What must I verify?", "Keep unknowns visible and turn them into next questions."]].map(([icon, title, copy]) => <div className="flex gap-4 rounded-xl border border-stone-200 p-4" key={title}><span className="grid h-10 w-10 shrink-0 place-items-center rounded-md bg-teal-50 text-teal-900"><Icon name={icon as IconName} /></span><div><h3 className="font-semibold">{title}</h3><p className="mt-1 text-sm leading-6 text-stone-600">{copy}</p></div></div>)}</div></div><div className="grid grid-cols-2 gap-3 rounded-[1.5rem] bg-stone-100 p-3"><img src={mediaFixture.places[0].images[1]} alt="Haste Street listing interior" className="aspect-[4/5] h-full w-full rounded-xl object-cover" /><div className="grid gap-3"><img src={mediaFixture.places[1].images[2]} alt="Second Haste Street listing interior" className="aspect-square w-full rounded-xl object-cover" /><div className="rounded-xl bg-teal-950 p-5 text-white"><Icon name="duplicate" className="text-amber-300" /><p className="mt-4 font-semibold">3 photos reused across two units</p><p className="mt-2 text-xs leading-5 text-teal-100">A visual signal most listing sites leave unexplained.</p></div></div></div></div></section>
    <section className="mx-auto max-w-6xl px-5 py-10 sm:px-8"><Card className="flex flex-col justify-between gap-4 p-6 sm:flex-row sm:items-center"><div><p className="font-semibold">Try it without an account</p><p className="mt-1 text-sm text-stone-600">The Berkeley demo is ready immediately. Save favorites privately in this browser.</p></div><div className="flex flex-wrap items-center gap-3"><Badge tone={savedCount ? "confirmed" : "unknown"}>{savedCount} saved {savedCount === 1 ? "place" : "places"}</Badge><Button onClick={onStart}>Start comparison →</Button></div></Card></section>
  </main></>;
}
  ...
  return (
    <header className="border-b border-stone-200 bg-white">
      <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-4 px-5 py-4 sm:px-8">
        <div><p className="text-2xl font-semibold leading-none tracking-[-0.04em] text-teal-950 [font-family:Georgia,serif]">Stateside</p><p className="mt-1 text-[11px] tracking-wide text-stone-500">Understand before you commit</p></div>
        {screen === "landing" ? <nav className="hidden items-center gap-5 text-sm font-medium text-stone-600 md:flex" aria-label="Landing page"><a href="#how-it-works" className="hover:text-teal-900">How it works</a><a href="#what-you-get" className="hover:text-teal-900">What it does</a></nav> : <div className="hidden items-center gap-2 text-sm sm:flex" aria-label={`Step ${step} of 3`}>
