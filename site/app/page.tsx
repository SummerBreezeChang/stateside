...
function PhotoEvidence({ media, full = false }: { media: MediaPlace; full?: boolean }) {
  ...
  const postId = listing.sourceUrl?.match(/\/(\d+)\.html$/)?.[1];
  ...
  return <div>
    ...
    <p className="mt-1 text-xs leading-5 text-stone-500">{listing.sourceUrl ? <>Source: Craigslist post {postId} · collected {listingFixture.collectedAt} · <a className="font-semibold text-teal-900 underline" href={listing.sourceUrl} target="_blank" rel="noreferrer">view original</a></> : <>Source: dated public listing research · collected {listingFixture.collectedAt} · original URL unavailable</>}</p>
...
function CompareScreen({ onBack, onOpen, favorites, onToggleFavorite }: { onBack: () => void; onOpen: (index: number) => void; favorites: string[]; onToggleFavorite: (placeId: string) => void }) {
  ...
  const rows = useMemo(() => [
    ...
    { label: "Cross-listing check", icon: "duplicate" as IconName, emphasis: true, cells: analysis.places.map((place) => { const findings = listingFixture.cross_listing_findings.filter((finding) => finding.placeIds.includes(place.id)); return { value: findings.map((finding) => finding.finding).join(" "), evidence: "confirmed" as Evidence, note: findings.map((finding) => finding.whyItMatters).join(" ") }; }) },
