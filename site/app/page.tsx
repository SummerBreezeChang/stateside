...
function LandingScreen({ onStart, onGuide, savedCount }: { onStart: () => void; onGuide: () => void; savedCount: number }) {
  return <><div className="landing-hero-shell overflow-hidden" style={{ backgroundImage: "linear-gradient(180deg, rgba(5,47,46,1) 0%, rgba(5,47,46,.72) 8rem, rgba(5,47,46,0) 14rem), linear-gradient(90deg, rgba(5,47,46,.97) 0%, rgba(5,47,46,.88) 38%, rgba(5,47,46,.28) 72%, rgba(15,23,42,.12) 100%), url('/stateside-flow-v3.jpg')" }}><AppHeader screen="landing" onStart={onStart} />
...
function AppHeader({ screen, onStart }: { screen: Screen; onStart?: () => void }) {
  if (screen === "landing") {
    return <header className="bg-transparent text-white"><div className="mx-auto grid max-w-[1440px] grid-cols-[1fr_auto] items-center gap-2 px-3 py-5 sm:grid-cols-[1fr_auto_1fr] sm:px-8">
      <span aria-hidden="true" className="hidden sm:block" />
      <button type="button" onClick={() => { window.location.href = "/"; }} className="stateside-wordmark justify-self-start text-white drop-shadow-sm [font-family:var(--font-unbounded)] sm:justify-self-center" aria-label="Return to Stateside home">STATESIDE</button>
      <Button variant="secondary" onClick={onStart} className="justify-self-end whitespace-nowrap border-white/70 bg-white/95 px-3 text-xs text-teal-950 shadow-sm hover:bg-white sm:px-4 sm:text-sm">Start comparing</Button>
    </div></header>;
  }
